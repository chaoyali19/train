...
public class PreserveOtherServiceImpl implements PreserveOtherService {
    ...
    private String queryForStationId(String stationName, HttpHeaders httpHeaders) {
        ...
        Response<String> response = reQueryForStationId.getBody();
        
        // 添加响应结构校验
        try {
            if (response.getStatus() != 1) {
                PreserveOtherServiceImpl.LOGGER.error("[queryForStationId][Response error][stationName: {}, status: {}, message: {}]", 
                    stationName, response.getStatus(), response.getMsg());
                throw new RuntimeException("Station service response error: " + response.getMsg());
            }
            
            Object data = response.getData();
            if (data == null) {
                PreserveOtherServiceImpl.LOGGER.error("[queryForStationId][Response structure error][stationName: {}, error: data is null]", stationName);
                throw new RuntimeException("Response structure error: data is null");
            }
            
            // 检查数据是否为字符串类型
            if (!(data instanceof String)) {
                PreserveOtherServiceImpl.LOGGER.error("[queryForStationId][Response structure error][stationName: {}, error: data is not String, actual type: {}]", 
            ...
            if (stationId.trim().isEmpty()) {
                PreserveOtherServiceImpl.LOGGER.error("[queryForStationId][Response structure error][stationName: {}, error: stationId is empty]", stationName);
            ...
            PreserveOtherServiceImpl.LOGGER.error("[queryForStationId][Response structure validation error][stationName: {}, error: {}]", stationName, e.getMessage());
