...
def is_path_match(target_path, call_path):
    ...
    # 处理调用路径包含目标路径的情况（但排除过于宽泛的匹配）
    if target_path in call_path:
        # 确保不是过于宽泛的匹配
        # 例如：/api/v1/configservice/configs 不应该匹配 /api/v1/configservice/configs/something
        if target_path.endswith('/'):
            ...
            if len(call_parts) >= len(target_parts):
                return '/'.join(call_parts[:len(target_parts)]) == target_path
