...
public class BasicServiceImpl implements BasicService {
    ...
    public Response queryForStationId(String stationName, HttpHeaders headers) {
        ...
        try {
            ...
            if (data == null) {
                BasicServiceImpl.LOGGER.error("[queryForStationId][Response structure error][stationName: {}, error: data is null]", stationName);
                throw new RuntimeException("Response structure error: data is null");
            }
            
            // 检查数据是否为字符串类型（站点ID应该是字符串）
            if (!(data instanceof String)) {
                BasicServiceImpl.LOGGER.error("[queryForStationId][Response structure error][stationName: {}, error: data is not String, actual type: {}]", 
                    stationName, data.getClass().getSimpleName());
                throw new RuntimeException("Response structure error: data type mismatch");
            }
            
            String stationId = (String) data;
            if (stationId.trim().isEmpty()) {
                BasicServiceImpl.LOGGER.error("[queryForStationId][Response structure error][stationName: {}, error: stationId is empty]", stationName);
                throw new RuntimeException("Response structure error: stationId is empty");
            }
            
        } catch (Exception e) {
            BasicServiceImpl.LOGGER.error("[queryForStationId][Response structure validation error][stationName: {}, error: {}]", stationName, e.getMessage());
            throw new RuntimeException("Response structure validation error: " + e.getMessage());
        }
    ...
    public Map<String,String> checkStationsExists(List<String> stationNames, HttpHeaders headers) {
        ...
        try {
            ...
            if (data == null) {
                BasicServiceImpl.LOGGER.error("[checkStationsExists][Response structure error][stationNames: {}, error: data is null]", stationNames);
                throw new RuntimeException("Response structure error: data is null");
            }
            
            // 检查数据是否为Map类型
            if (!(data instanceof Map)) {
                BasicServiceImpl.LOGGER.error("[checkStationsExists][Response structure error][stationNames: {}, error: data is not Map, actual type: {}]", 
                    stationNames, data.getClass().getSimpleName());
                throw new RuntimeException("Response structure error: data type mismatch");
            }
            
            Map<String, String> stationMap = (Map<String, String>) data;
            
            // 检查Map中的值是否都是字符串类型
            for (Map.Entry<String, String> entry : stationMap.entrySet()) {
                if (entry.getValue() == null) {
                    BasicServiceImpl.LOGGER.error("[checkStationsExists][Response structure error][stationNames: {}, error: stationId is null for station: {}]", 
                        stationNames, entry.getKey());
                    throw new RuntimeException("Response structure error: stationId is null");
                }
                if (!(entry.getValue() instanceof String)) {
                    BasicServiceImpl.LOGGER.error("[checkStationsExists][Response structure error][stationNames: {}, error: stationId is not String for station: {}, actual type: {}]", 
                        stationNames, entry.getKey(), entry.getValue().getClass().getSimpleName());
                    throw new RuntimeException("Response structure error: stationId type mismatch");
                }
                if (((String) entry.getValue()).trim().isEmpty()) {
                    BasicServiceImpl.LOGGER.error("[checkStationsExists][Response structure error][stationNames: {}, error: stationId is empty for station: {}]", 
                        stationNames, entry.getKey());
                    throw new RuntimeException("Response structure error: stationId is empty");
                }
            }
            ...
            BasicServiceImpl.LOGGER.error("[checkStationsExists][Response structure validation error][stationNames: {}, error: {}]", stationNames, e.getMessage());
            throw new RuntimeException("Response structure validation error: " + e.getMessage());
        }
    }
