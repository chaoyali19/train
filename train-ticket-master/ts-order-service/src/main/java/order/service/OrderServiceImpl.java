...
import order.controller.FaultController;
...
public class OrderServiceImpl implements OrderService {
    ...
    private static final int DEFAULT_DEMO_USERS_LIMIT = 20000;
    ...
    public Response getSoldTickets(Seat seatRequest, HttpHeaders headers) {
        if (FaultController.isSlowSQLEnabled()){
            // 通过API调用获取demo_users表数据
            for (int i = 0; i < 3; i++) {
                LOGGER.info("[Query demo_users table via API]");
//            String url = "http://127.0.0.1:8080" + "/api/v1/demouserservice/users/top?limit=10000";
                String url = getServiceUrl("ts-order-service") + "/api/v1/demouserservice/users/top?limit=100000";
                ResponseEntity<Response<List<DemoUser>>> responseEntity = restTemplate.exchange(
                        url,
                        HttpMethod.GET,
                        null,
                        new ParameterizedTypeReference<Response<List<DemoUser>>>() {}
                );
                Response<List<DemoUser>> apiResponse = responseEntity.getBody();
                List<DemoUser> demoUsers = apiResponse != null ? apiResponse.getData() : Collections.emptyList();
                LOGGER.info("[Demo users query result][size: {}][{}] [{}]", demoUsers.size(), i, url);
            }
        }
        ...
        String urlPrefix = System.getenv("EX_URL");
        if (urlPrefix != null && !urlPrefix.isEmpty() && FaultController.isExternalApiEnabled()) {
            ...
            OrderServiceImpl.LOGGER.info("[aaa][Left ticket info][info is : {}]",  re3.getBody().getData().toString());
        }
