    ...
    private String queryForStationId(String stationName, HttpHeaders httpHeaders) {
        ...
        try {
            if (response.getStatus() != 1) {
                PreserveServiceImpl.LOGGER.error("[queryForStationId][Response error][stationName: {}, status: {}, message: {}]", 
            ...
            if (data == null) {
                PreserveServiceImpl.LOGGER.error("[queryForStationId][Response structure error][stationName: {}, error: data is null]", stationName);
                throw new RuntimeException("Response structure error: data is null");
            }
            
            // 检查数据是否为字符串类型
            if (!(data instanceof String)) {
                PreserveServiceImpl.LOGGER.error("[queryForStationId][Response structure error][stationName: {}, error: data is not String, actual type: {}]", 
            ...
            if (stationId.trim().isEmpty()) {
                PreserveServiceImpl.LOGGER.error("[queryForStationId][Response structure error][stationName: {}, error: stationId is empty]", stationName);
            ...
            PreserveServiceImpl.LOGGER.error("[queryForStationId][Response structure validation error][stationName: {}, error: {}]", stationName, e.getMessage());
